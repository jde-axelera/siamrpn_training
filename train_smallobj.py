...
# ── Scale-drop augmentation wrapper ──────────────────────────────────────────
class ScaleDropWrapper(Dataset):
    """
    Wraps any SiamRPN dataset to randomly apply scale-drop augmentation.

    Scale-drop: synthetically shrinks the target in the search frame by
    expanding the search crop and resizing back to 255px.  This trains
    the network to correlate a large template with a small search target —
    exactly the failure mode observed in ir_crop.mp4 (landing approach).

    Probability controlled by cfg.DATASET.SCALE_DROP_PROB (default 0.4).
    Scale range: cfg.DATASET.SCALE_DROP_MIN – cfg.DATASET.SCALE_DROP_MAX.
    """
    ...
    def __getitem__(self, idx):
        ...
        prob    = getattr(cfg.DATASET, 'SCALE_DROP_PROB', 0.60)
        sf_min  = getattr(cfg.DATASET, 'SCALE_DROP_MIN',  0.15)
        sf_max  = getattr(cfg.DATASET, 'SCALE_DROP_MAX',  0.8)

        if random.random() > prob:
            return item   # no augmentation

        sf = random.uniform(sf_min, sf_max)   # target will appear sf× smaller

        # ── expand search crop so target fills sf× less space ────────────────
        search = item['search']                # float32 (3, 255, 255)
        s_np   = np.clip(search.transpose(1, 2, 0), 0, 255).astype(np.uint8)

        new_sz  = max(int(255 / sf), 256)      # expanded size
        pad     = (new_sz - 255) // 2
        avg_col = [float(s_np[:,:,c].mean()) for c in range(3)]
        s_exp   = cv2.copyMakeBorder(s_np, pad, pad, pad, pad,
                                     cv2.BORDER_CONSTANT, value=avg_col)
        s_scaled = cv2.resize(s_exp, (255, 255)).transpose(2, 0, 1).astype(np.float32)

        # ── remap ground-truth bbox ──────────────────────────────────────────
        orig_bbox = item['bbox']               # [x1, y1, x2, y2] in 255px coords
        cx  = (orig_bbox[0] + orig_bbox[2]) / 2.0
        cy  = (orig_bbox[1] + orig_bbox[3]) / 2.0
        w   = (orig_bbox[2] - orig_bbox[0]) * sf
        h   = (orig_bbox[3] - orig_bbox[1]) * sf
        new_cx = (cx - 127.5) * sf + 127.5
        new_cy = (cy - 127.5) * sf + 127.5
        new_bbox = center2corner(Center(new_cx, new_cy, w, h))

        cls, delta, delta_weight, _ = self.anchor_target(
            new_bbox, cfg.TRAIN.OUTPUT_SIZE, neg=False)
        ...
        return {
            ...
            'label_loc':        delta,
            'label_loc_weight': delta_weight,
            'bbox':             np.array(new_bbox),
        }


# ── DIoU regression loss ─────────────────────────────────────────────────────
def diou_loss(pred_boxes, target_boxes, weights=None, eps=1e-7):
    """
    Distance-IoU loss on [x1,y1,x2,y2] tensors.
    Handles degenerate boxes (zero area) gracefully.
    """
...
# ── Size-weighted cross-entropy ───────────────────────────────────────────────
def size_weighted_cls_loss(pred, label, weight, bbox, max_size_weight=5.0):
    """
    Weighted BCE classification loss with additional inverse-area weighting.
    Smaller targets get up to max_size_weight× higher gradient contribution.

    bbox: (B, 4) in search-crop pixel coords [x1,y1,x2,y2]
    """
    # Per-sample target area (search crop pixels)
    bw = (bbox[:, 2] - bbox[:, 0]).clamp(min=1.0)
    bh = (bbox[:, 3] - bbox[:, 1]).clamp(min=1.0)
    area = bw * bh                                    # (B,)
    ref_area = 32.0 * 32.0
    size_w = (ref_area / area).clamp(1.0, max_size_weight)  # (B,)
    size_w = size_w / size_w.mean()                   # normalise so mean=1

    # Standard weighted cross-entropy
    pred_flat   = pred.reshape(-1, pred.shape[-1]) if pred.dim() > 2 else pred
    label_flat  = label.reshape(-1)
    weight_flat = weight.reshape(-1)

    # Apply size weight per sample (broadcast over anchor grid)
    B = bbox.shape[0]
    spatial = pred_flat.shape[0] // B
    size_w_expanded = size_w.unsqueeze(1).expand(B, spatial).reshape(-1)

    mask    = weight_flat != -1
    ce_loss = F.cross_entropy(pred_flat[mask], label_flat[mask].long(),
                               reduction='none')
    sw      = size_w_expanded[mask] * weight_flat[mask].clamp(min=0)
    return (ce_loss * sw).sum() / (sw.sum() + 1e-8)
...
def run_epoch(model, loader, device, optimizer=None, epoch=0):
    ...
    with ctx:
        ...
        for step, data in enumerate(loader):
            ...
            outputs = model(batch)
            # ── Small-object: override cls_loss with size-weighted version ──
            if training and "label_cls" in batch and "bbox" in batch:
                sw_cls = size_weighted_cls_loss(
                    outputs.get("cls", outputs.get("cls_logits",
                        # fallback: recompute from outputs dict if raw cls not stored
                        outputs.get("cls_loss", torch.tensor(0., device=device)).unsqueeze(0)
                    )),
                    batch["label_cls"],
                    batch.get("label_cls_weight", torch.ones_like(batch["label_cls"])),
                    batch["bbox"],
                    max_size_weight=getattr(cfg.TRAIN, "SIZE_WEIGHT_MAX", 5.0),
                ) if outputs.get("cls") is not None else outputs["cls_loss"]
                loss = (cfg.TRAIN.CLS_WEIGHT * sw_cls
...
def main():
    ...
    # ── Wrap each dataset with ScaleDropWrapper for small-object augmentation ──
    _all_ds_vars = [
    ...
    (antiuav_train, msrs_train, vtmot_train, massmind_train, mvss_train,
     dutvtuav_train, antiuav300_train, birdsai_train, hituav_train) = [
        ScaleDropWrapper(d) for d in _all_ds_vars
    ]
    ...
    if resume and os.path.isfile(resume):
        ...
        try:
            ...
            logger.warning(f"Skipping optimizer state (mismatch): {e} — starting fresh optimizer")

...
class Augmentation:
    def __init__(self, shift, scale, blur, flip, color, scale_one_sided=False):
        self.shift = shift
        self.scale = scale
        self.scale_one_sided = scale_one_sided
        self.blur = blur
        self.flip = flip
        self.color = color
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size):
        ...
        if self.scale:
            _r = Augmentation.random
            scale_x = (1.0 + (abs(_r()) if self.scale_one_sided else _r()) * self.scale)
            scale_y = (1.0 + (abs(_r()) if self.scale_one_sided else _r()) * self.scale)
